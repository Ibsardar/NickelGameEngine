...
__nhelpers = {
    ...
    determine_destination : function(g, d) { return d instanceof Array ? g.map[d[0]][d[1]] : d; }, /* specialized function */
    is_array : function(o) { return o instanceof Array },
    assign_id : function() { return Nickel.ID++; }, /* specialized function */
    ...
    scalar_product : function(s, v) { return [s * v[0], s * v[1]]; },
    add_scalar : function(s, v) { return [s + v[0], s + v[1]]; },
    copy_vector : function(v) { return [v[0], v[1]]; },
    is_zero : function(v) {  return !v[0] && !v[1] ? true : false; },
    atan2_degrees : function(a, b) { return Nickel.UTILITY.atan2(a,b) * 180 / Math.PI },
    scalar_divide : function(s, v) { return [v[0]/s, v[1]/s]; },
    is_equal : function(a, b) { return a[0] === b[0] && a[1] === b[1]; },
    vector_divide : function(v1, v2) { return [v1[0] / v2[0], v1[1] / v2[1]]; },
    vector_multiply : function(v1, v2) { return [v1[0] * v2[0], v1[1] * v2[1]]; },
    sleep : function(ms) { var a=Date.now(); while (Date.now()<a+ms) {} },
...
var Nickel = {
    ...
    *  - global id counter
    *  - is sgame started (used only in update.js)
    *  - store globals here instead of window
    */
    SCORE     : 0,
    ID        : 1000,
    STARTED   : false,
    ...
    */
    UTILITY   : {
        random_element :        __nhelpers.random_element,
        random_number :         __nhelpers.random_number,        
        round :                 __nhelpers.round,
        determine_destination : __nhelpers.determine_destination,                
        is_array :              __nhelpers.is_array,
        assign_id :             __nhelpers.assign_id,    
        magnitude_of_vector :   __nhelpers.magnitude_of_vector,            
        normalize_vector :      __nhelpers.normalize_vector,        
        vector_product :        __nhelpers.vector_product,        
        cross_product :         __nhelpers.cross_product,        
        dot_product :           __nhelpers.dot_product,    
        add_vector :            __nhelpers.add_vector,    
        subtract_vector :       __nhelpers.subtract_vector,        
        trim_angle :            __nhelpers.trim_angle,    
        atan2 :                 __nhelpers.atan2,
        midpoint :              __nhelpers.midpoint,
        scalar_product :        __nhelpers.scalar_product,        
        add_scalar :            __nhelpers.add_scalar,    
        copy_vector :           __nhelpers.copy_vector,    
        is_zero :               __nhelpers.is_zero,
        atan2_degrees :         __nhelpers.atan2_degrees,        
        scalar_divide :         __nhelpers.scalar_divide,        
        is_equal :              __nhelpers.is_equal,
        vector_divide :         __nhelpers.vector_divide,        
        vector_multiply :       __nhelpers.vector_multiply,        
    ...
    // utility proxies
    util : {
        r_elem :        __nhelpers.random_element,
        r_num :         __nhelpers.random_number,
        round :         __nhelpers.round,
        is_arr :        __nhelpers.is_array,
    ...
    // 2D vector proxies
    v2d : {
        is :        __nhelpers.is_array,
        is_0 :      __nhelpers.is_zero,
        mag :       __nhelpers.magnitude_of_vector,
        norm :      __nhelpers.normalize_vector,
        mult :      __nhelpers.vector_product,
        mult_v :    __nhelpers.vector_multiply,
        div :       __nhelpers.vector_divide,
        cross :     __nhelpers.cross_product,
        dot :       __nhelpers.dot_product,
        add :       __nhelpers.add_vector,
        sub :       __nhelpers.subtract_vector,
        mid :       __nhelpers.midpoint,
        add_s :     __nhelpers.add_scalar,
    ...
    // * note: expects images like: {img:"...src...", w:#, h:#}
    // image processing proxies
    ip : {
        blur :      __nhelpers.blur_rgb_image,
        blur_a :    __nhelpers.blur_rgba_image,
        blur_cp :   __nhelpers.get_blur_rgb_image,
        blur_a_cp : __nhelpers.get_blur_rgba_image
    },//end .ip shortcut
...
////////////////////////////////////////////
///   COLLISION DETECTOR   ///////////////// ...TODO: CLEAN COMMENTS, PRINT STATEMENTS
////////////////////////////////////////////
//
...
var Collision_Detector = {
    ...
    //TODO: FIX NaNs WHEN LINE IS VERTICAL <-CONFIRMED RELATED GLITCH on 6/18/2019
    ...
    //TODO: OPTIMIZE, FIX FALSE COLLISION DETECTIONS
    ...
    // TODO: OPTIMIZE, FIX FALSE COLLISION DETECTION
    collides_poly_point : function (me, you) {
        ...
        for (var i=0, j=1; i<me.vertices.length; i++, j++) {
            ...
            if (Collision_Detector.collides_ray_line(ray, edge)) {
                ...
                // edge case: ray aligns with a flat edge
                // - if intersection: collision
                // - if no intersection: no collision (don't increment `collisions`)
                if (you[1] == edge.y && edge.y == edge.yend) {
                    if (Collision_Detector.collides_line_point(edge, you))
                        return true;

                // normal case: collision
                } else
                    collisions++;
            }
        }
    ...
    collides_circle_circle : function (me, you, resolve_me=false, resolve_you=false,
        ...
        if (dist < sum) {
            ...
            if (resolve_me || resolve_you) {
                ...
                // default if normalization not possible (probably zero-vector)
                if (Nickel.v2d.is_0(unit_dir_vec))
...
}//end Collision_Detector namespace
...
function Viewport(html_canvas_element_id) {
    ...
    this.run = function() {
        ...
        // relatively globalize 'this' for use below
        self = this;
    ...
    this.copy = function() {
        //--    Returns an identical canvas object
        //--
    ...
    this.resume = function() {
        //--    Resumes viewport after being paused
        //--
    ...
    this.reset_key = function(keycode) {
        //--    Resets a certain key's value to false
        //--

        this.key_downs[keycode] = false;
    }
    ...
    this.get_data = function() {
        //--    Copies and returns image data from canvas
        //--

        return this.context.getImageData(0, 0, this.get_w(), this.get_h());
    }

    this.set_data = function(data) {
        //--    Paints image or canvas data onto the canvas
        //--
...
////////////////////////////////////////////
///   SIMPLE TIMER   ///////////////////////    *** todo: needs testing ***
////////////////////////////////////////////
function SimpleTimer() {
    ...
    this.update = function() {
        //-- Updates the timer and its state.
        //--
    ...
    this.set_alarm = function(milliseconds) {
        //-- Sets an alarm to go off in some amount of milliseconds.
        //--
    ...
    this.unpause = function() {
        //-- Unpauses the timer and updates the end time.
        //--

        this.limit = Date.now() + this.remaining();
        this.paused = false;
    }

    this.remaining = function() {
        //-- Returns how much time is left till the end.
        //--
...
function SimpleText(scene, text, font_family='sans-serif', size=12, color='black',
                     pos=[0,0], alignment="left", weight="normal", valign="middle") {
    ...
    this.draw = function() {
        ...
        // copy antialiasing property
        ctx.imageSmoothingEnabled = this.scene.antialias;
    ...
    this.set_outline = function(color=null, thickness=-1) {
        //--    Change the text-outline color, thickness, or both
        //--
    ...
    Object.defineProperty(this, "center", {
        ...
        set : function (c) { this.x = c[0]; this.y = c[1]; }
    });
...
function SimpleImage(scene, source, w=0, h=0, pos=[0,0],
    ...
    this.draw = function() {
        ...
        if (this.image) {
            if (this.cropped)
                // (params: img, cx,cy,cw,ch, destx,desty,destw,desth)
                ctx.drawImage(
                    this.image,
                    this._crop.x, this._crop.y, this._crop.w, this._crop.h,
                ...
                ctx.drawImage(
                    this.image,
                    this._crip.x, this._crip.y, this._crip.w, this._crip.h,
                    this.x + this._crip.x, this.y + this._crip.y, this._crip.w, this._crip.h
                );
            else
                // (params: img,x,y,w,h)
                ctx.drawImage(
    ...
    this.set_image = function(source, w=this.width, h=this.height, color='', outline='', outline_thickness=0) {
        //--    Change the image & image size OR rect size, rect color, & rect outline
        //--
    ...
    this.crop = function(ltrim=0, rtrim=0, ttrim=0, btrim=0) {
        //--    Dedicated crop. Returns false with a warning if invalid.
        //--

        // validate
        if (this.cropped) {
            console.warn('Warning: SimpleImage>crop: Already cropped. Crop ignored.');        
            return false;
        }
        if (this.cripped) {
            console.warn('Warning: SimpleImage>crop: Already cripped. Crop ignored.');        
            return false;
        }
        if (ltrim < 0 || rtrim < 0 || ttrim < 0 || btrim < 0 ||
            ltrim+rtrim > this.width || ttrim+btrim > this.height) {
            console.warn('Warning: SimpleImage>crop: Invalid crop parameters. Crop ignored.');        
            return false;
        }
        ...
        this._crop.x = ltrim;
        this._crop.y = ttrim;
        this._crop.w = this.width - ltrim - rtrim;
        this._crop.h = this.height - ttrim - btrim;
        this.width = this._crop.w;
        this.height = this._crop.h;
        this.cropped = true;
    }

    this.uncrop = function() {
        //--    Reverts cropped image back to original image.
        //--
    ...
    this.crip = function(ltrim=0, rtrim=0, ttrim=0, btrim=0) {
        //--    Crop-in-place. Returns false with a warning if invalid.
        //--

        // validate
        if (this.cropped) {
            console.warn('Warning: SimpleImage>crip: Already cropped. Crip ignored.');        
            return false;
        }
        if (this.cripped) {
            console.warn('Warning: SimpleImage>crip: Already cripped. Crip ignored.');        
            return false;
        }
        if (ltrim < 0 || rtrim < 0 || ttrim < 0 || btrim < 0 ||
            ltrim+rtrim > this.width || ttrim+btrim > this.height) {
            console.warn('Warning: SimpleImage>crip: Invalid crip parameters. Crip ignored.');        
            return false;
        }
        ...
        this._crip.x = ltrim;
        this._crip.y = ttrim;
        this._crip.w = this.width - ltrim - rtrim;
        this._crip.h = this.height - ttrim - btrim;
        this.cripped = true;
    }

    this.uncrip = function() {
        //--    Reverts cripped image back to original image.
        //--
...
////////////////////////////////////////////
///   SIMPLE HORIZ. LOAD BAR   /////////////
////////////////////////////////////////////
function SimpleHLoadBar(scene, img_under, img_over, w=0, h=0, pos=[0,0], l2r=true, percent=0) {
    
    // general
    this.id = Nickel.UTILITY.assign_id();
    this.type = 'SimpleHLoadBar';
    this.scene = scene;
    this.canvas = scene.canvas;
    this.context = this.canvas.getContext("2d");

    // dims same for all imgs
    this.width = w;
    this.height = h;

    // images
    this.under = img_under;
    this.over = img_over;
    ...
    // start position of loading (l2r or r2l)
    this.l2r = l2r;

    // start percent of 'over' painted on top of 'under' from load directon
    this.percent = percent;
    this._prev_perc = percent;
    ...
    this.move = function() {
        //-- Handles any image changes regarding position, dimension, scale
        //--

        // change dimension changes
        var dim_changed = (this.over.width != this.width || this.over.height != this.height);

        this.under.x = this.x;
        this.under.y = this.y;
        this.under.width = this.width;
        this.under.height = this.height;
        this.over.x = this.x;
        this.over.y = this.y;
        this.over.width = this.width;
        this.over.height = this.height;

        // match over bar to new percent (if applicable)
        // also re-match if width/height changed
        if (this._prev_perc != this.percent || dim_changed)
            this._adjust_over_bar();

        // record percent
        this._prev_perc = this.percent;
    }
    ...
    this.handle_events = function() {
        ...
        // on percent change
        if (this._prev_perc != this.percent) {
            this.on_change(this._prev_perc, this.percent);

            // on 0 percent
            if (this.percent <= 0)
                this.on_empty(this._prev_perc);

            // on 100 percent
            if (this.percent >= 100)
                this.on_full(this._prev_perc);
        }
    }
    ...
    this._adjust_over_bar = function() {
        //--    Crop over bar:
        //--

        // - calculate (over.width never changes)
        var new_w = this.over.width * this.percent * 0.01;
        ...
        if (this.l2r) {
            ...
            // crop in place
            this.over.crip(0, this.over.width - new_w);

        // - right to left
        } else {

            // crop in place
            this.over.crip(this.over.width - new_w, 0);
    ...
    this.on_empty = function(previous) {
        //--    Called in event handler. Meant to be over-ridden.
        //--

    }

    this.on_full = function(previous) {
        //--    Called in event handler. Meant to be over-ridden.
        //--
        
    }

    this.on_change = function(previous, current) {
        //--    Called in event handler. Meant to be over-ridden.
        //--
    ...
    this.set_l2r = function() {
        //--    Makes loadbar load from left to right.
        //--    
    ...
    this.set_r2l = function() {
        //--    Makes loadbar load from right to left.
        //--    
    ...
    //
    // - initially set correct over bar cropping
    this._adjust_over_bar();
...
////////////////////////////////////////////
///   SIMPLE VERT. LOAD BAR   //////////////
////////////////////////////////////////////
function SimpleVLoadBar(scene, img_under, img_over, w=0, h=0, pos=[0,0], t2b=false, percent=0) {
    
    // general
    this.id = Nickel.UTILITY.assign_id();
    this.type = 'SimpleVLoadBar';
    this.scene = scene;
    this.canvas = scene.canvas;
    this.context = this.canvas.getContext("2d");
    ...
    // start position of loading (t2b or b2t)
    this.t2b = t2b;
    ...
    this._adjust_over_bar = function() {
        ...
        // - calculate (over.height never changes)
        var new_h = this.over.height * this.percent * 0.01;
        ...
        // - top to bottom
        if (this.t2b)
            this.over.crip(0, 0, 0, this.over.height - new_h); //crop in place

        // - bottom to top
        else
            this.over.crip(0, 0, this.over.height - new_h, 0); //crop in place
    ...
    this.set_t2b = function() {
        //--    Makes loadbar load from top to bottom.
        //--    
    ...
    this.set_b2t = function() {
        //--    Makes loadbar load from bottom to top.
        //--    
...
///   SIMPLE PANEL   ///////////////////////
////////////////////////////////////////////
function SimplePanel(scene, img, w=0, h=0, pos=[0,0], rows=0, cols=0, vrows=0, vcols=0, padx=0, pady=0) {

    // general
    this.id = Nickel.UTILITY.assign_id();
    this.type = 'SimplePanel';
    this.scene = scene;
    this.canvas = scene.canvas;
    this.context = this.canvas.getContext("2d");
    ...
    for (var i=0; i<rows; i++)
        this.grid.data[i] = Array(cols);

    // view
    this.view = {
        x:  0, // positive means scrolled to the right
        y:  0, // positive means scrolled downwards
        w:  vcols,
        h:  vrows
    }
    ...
    this.update_grid = function() {
        ...
        // warnings
        if (!this.grid.w || !this.grid.h || !this.view.w || !this.view.h) {
            console.warn('Warning: SimplePanel>update_grid: Panel is empty');
            return;
        }

        // available space
        var height_avail = this.height - this.pady - this.pady;
        var width_avail = this.width - this.padx - this.padx;

        // calculate y-padding
        var ypad = height_avail / this.view.h / 2;
        var ystep = this.y + this.pady;

        // get starting grid position
        var start_row = this.view.y;
        var start_col = this.view.x;

        // get iteration amounts
        var row_iters = Math.min(this.grid.h - start_row, this.view.h);
        var col_iters = Math.min(this.grid.w - start_col, this.view.w);

        // for each row in view, calculate x-padding, update
        for (var i=start_row, rcnt=0; rcnt<row_iters; i++, rcnt++) {
            var row = this.grid.data[i];

            // pre pad
            ystep += ypad;
            var xpad = width_avail / this.view.w / 2;
            var xstep = this.x + this.padx;

            // for each item in row, set center, update
            if (row) {
                for (var j=start_col, ccnt=0; ccnt<col_iters; j++, ccnt++) {
                    var ui_item = row[j];
    ...
    this.data_at = function(pos) {
        //--    Returns item at grid position
        //--

        try { return this.grid.data[pos[1]][pos[0]]; }
        catch(e) { console.error('Error: SimplePanel>data_at: ' + e); }
    }

    this.view_at = function(pos) {
        //--    Returns item at view position
        //--

        try { return this.grid.data[pos[1]+this.view.y][pos[0]+this.view.x]; }
        catch(e) { console.error('Error: SimplePanel>view_at: ' + e); }
    }

    this.set_at = function(ui_item, pos) {
        //--    Adds/replaces a UI item at a grid position
        //--

        try { this.grid.data[pos[1]][pos[0]] = ui_item; }
        catch(e) { console.error('Error: SimplePanel>set_at: ' + e); }
    }

    this.unset_at = function(pos) {
        //--    Removes a UI item at a grid position
        //--

        try { this.grid.data[pos[1]][pos[0]] = null; }
        catch(e) { console.error('Error: SimplePanel>unset_at: ' + e); }
    }

    this.unset_row = function(row) {
        //--    Removes all UI items in a row
        //--

        try { this.grid.data[row] = Array(this.grid.w); }
        catch(e) { console.error('Error: SimplePanel>unset_row: ' + e); }
    }

    this.unset_col = function(col) {
        //--    Removes all UI items in a column
        //--

        try { for (var i=0; i<this.grid.h; i++) this.grid.data[i][col] = null; }
        catch(e) { console.error('Error: SimplePanel>unset_col: ' + e); }
    }

    this.unset = function() {
        //--    Removes all UI items
        //--

        try { for (var i=0; i<this.grid.h; i++) this.grid.data[i] = Array(this.grid.w); }
        catch(e) { console.error('Error: SimplePanel>unset: ' + e); }
    }

    this.free_scroll = function(x=0, y=0) {
        //--    Scrolls the grid within the view.
        //--    Scrolling is unbounded
        //--

        this.view.x += x * this._inv_x;
        this.view.y += y * this._inv_y;
    }

    this.scroll = function(x=0, y=0) {
        //--    Scrolls the grid within the view.
        //--    Scrolling is bounded when grid-edge reaches view-edge.
        //--    Down/right is positive and vice-versa
        //--

        // get available space
        var av_l = Math.max(0, this.view.x);
        var av_r = Math.max(0, this.grid.w - this.view.w - this.view.x);
        var av_t = Math.max(0, this.view.y);
        var av_b = Math.max(0, this.grid.h - this.view.h - this.view.y);
        ...
        // scroll
        this.free_scroll(x, y);
    }

    this.unscroll = function() {
        //--    Resets scrolling
        //--
    ...
    this.scroll_up = function(y=1) {
        //--    Scrolls the grid upward within the view.
        //--    Scrolling is bounded when grid-edge reaches view-edge
        //--
    ...
    this.scroll_down = function(y=1) {
        //--    Scrolls the grid downward within the view.
        //--    Scrolling is bounded when grid-edge reaches view-edge
        //--
    ...
    this.scroll_right = function(x=1) {
        //--    Scrolls the grid to the right within the view.
        //--    Scrolling is bounded when grid-edge reaches view-edge
        //--
    ...
    this.scroll_left = function(x=1) {
        //--    Scrolls the grid to the left within the view.
        //--    Scrolling is bounded when grid-edge reaches view-edge
        //--
    ...
    this.invert = function(xaxis=true, yaxis=true) {
        //--    Toggles inversion of scrolling
        //--

        this._inv_x *= (xaxis ? -1 : 1);
        this._inv_y *= (yaxis ? -1 : 1);
    }

    this.inverted_xaxis = function() {
        //--    Returns true/false if inverted/not on x-axis
        //--

        return this._inv_x == 1;
    }

    this.inverted_yaxis = function() {
        //--    Returns true/false if inverted/not on y-axis
        //--

        return this._inv_y == 1;
    }
...
function SimplePoly(scene, vertices, is_equiangular=false, track_point=null) {
    ...
    this.set_vertices = function(verts, ref=false) {
        //--    Sets a new set of vertices
        //--
    ...
    this.get_bounds = function(alt=false) {
        //--    Returns the bounds of the polygon in the
        //--    format: {left:l#, right:r#, top:t#, bottom:b#}
        //--
        //--    if alt is 1, returns bounds in the
        //--    format: {x:l#, y:t#, w:w#, h:h#}
        //--
        //--    if alt is 2, returns bounds in the
        //--    format: [l#, t#, w#, h#]
        //--
        ...
        if (alt == 1) {
            ...
            bounds.x = Math.min(...vx);
            bounds.y = Math.min(...vy);
            bounds.w = Math.max(...vx) - bounds.x;
            bounds.h = Math.max(...vy) - bounds.y;
        }

        else if (alt == 2) {
            bounds = []
            bounds.push(Math.min(...vx));
            bounds.push(Math.min(...vy));
            bounds.push(Math.max(...vx) - bounds.x);
            bounds.push(Math.max(...vy) - bounds.y);
        }
        
        else {
            bounds = {left:0, right:0, top:0, bottom:0}
            bounds.left   = Math.min(...vx);
            bounds.top    = Math.min(...vy);
            bounds.right  = Math.max(...vx);
            bounds.bottom = Math.max(...vy);
        }
...
function SimpleCircle(scene, radius, track_point=null) {
    ...
    this.get_bounds = function(alt=false) {
        //--    Returns the bounds of the circle in the
        //--    format: {left:l#, right:r#, top:t#, bottom:b#}
        //--
        //--    if alt is 1, returns bounds in the
        //--    format: {x:l#, y:t#, w:w#, h:h#}
        ...
        if (alt == 1) {
            ...
            bounds = {
                ...
                right: this.cx + this.radius,
                top: this.cy - this.radius,
                bottom: this.cy + this.radius
            }
        }
...
function BoundingBox(sprite) {
    ...
    this.bound = function(force_bound=false) {
        ...
        // ignore this function if we already updated
        // and if we are not forcing it
...
function ColliderHull(sprite, approximate=true) {
    ...
    this.update_transformations = function() {
        ...
        if (!this.updated) {
            ...
            // TODO: OPTIMIZE CHECKING TYPES -> TODO (AS WELL AS MANY OTHER PLACES IN THIS FILE)
            ...
            /***   TODO: CHECK IF STILL A PROBLEM: ERROR - TRACKER POINT SHOULD BE CENTER OF LOCOMOTIVES (CENTER NOT EQUAL TO TRACKER PT!!!)
    ...
    this.approximate_shape = function() {
        //--    Approximates a rectangle to bound the sprite with
        //--
...
function Sprite(scene, image_data, has_bbox=true,
    ...
    // general
    this.id = Nickel.UTILITY.assign_id();
    this.type = 'Sprite';   // hash this string into an int for efficiency ***TODO*** <-- UNNECESSARY!
    this.scene = scene;
    this.canvas = scene.canvas;
    this.context = this.canvas.getContext("2d");
    ...
    this.update = function() {
        ...
        // update before any changes are made
        this.update_before();
    ...
    this.draw = function() {
        ...
        ctx.translate(this.x+this.origin[0], this.y+this.origin[1]);
        ctx.rotate(rot);
    ...
    this.get_scale = function() {
        //--    Returns image scale in both directions
        //--
    ...
    this.get_x2 = function() {
        ...
        //--    *BROKEN: ONLY WORKS IF NOT ROTATING/SCALING
        //--
    ...
    this.get_pos2 = function() {
        ...
        //--    *BROKEN: ONLY WORKS IF NOT ROTATING/SCALING
        //--    FIX VERY CAREFULLY SINCE THIS IS USED A LOT
        //--
    ...
    this.flip_h = function(flip_origin=false) {
        ...
        // vars
        var i = this.image;
        var w = this.get_w_orig();
        var h = this.get_h_orig();
        var c = document.createElement("canvas");
        var ctx = c.getContext("2d");
        var flip = () => {
            c.width = w;
        ...
        // flip
        if (i.complete) flip();
        else i.addEventListener('load', function tmp() {
            flip();
            i.removeEventListener('load', tmp);
        });
    }
    ...
    this.set_scale2 = function(nx, ny) {
        //--    Scale sprite by nx, ny
        //--
    ...
    this.set_velocity = function(v, instant=false) {
        ...
        //--    Turns gradually rather than instantly by default
        //--
    ...
    this.add_velocity = function(v, instant=false, calculated_v=null) {
        //--    Adds to the accurate velecity vector.
        //--    Turns gradually rather than instantly by default.
        //--    Can be given a calculated current velocity to
        //--    improve efficiency.
        //--

        // add velocity
        var vnew;
        if (calculated_v) vnew = Nickel.v2d.add(calculated_v, v);
        else vnew = Nickel.v2d.add(this.get_velocity(), v);

        // set new velocity
        this.set_velocity(vnew, instant);
    }
    ...
    this.copy_base = function(do_update=true) {
        ...
        //spr.update_before = this.update_before;
    ...
    this.offset_scale = function(scale,origin) {
        ...
        //--    Only accepts 1 scaling dimension
        //--
    ...
    this.offset_scale2 = function(scale,origin) {
        //--    Offsets scale via scaling matrix calculations.
        //--    Accepts 2D x and y scaling dimension
        //--

        var ox_perc = this.origin[0] / this.get_w();
        var oy_perc = this.origin[1] / this.get_h();

        this.set_x(scale[0]*(this.get_x()-origin[0]) + origin[0]);
        this.set_y(scale[1]*(this.get_y()-origin[1]) + origin[1]);
...
function SpriteSelector(scene) {
    ...
    this.unselect = function(sel_type, spr) {
        ...
        //delete this.selection_dictionary[sel_type][spr.get_type()][id];    //// MIGHT IMPACT PERFORMANCE BADLY <TODO: CHECK>  MAY NOT NEED !!!
    }
...
////////////////////////////////////////////
///   QUADTREE  ////////////////////////////        // TODO: REMOVE OVERLAP FEATURE MAYBE (NOT RELIABLE ANYMORE)...?
////////////////////////////////////////////
...
function TreeNode(obj=null) {
    ...
    // index of self in parent's children list (-1 means root)
    this.index = -1;

    // list of children TreeNodes
    this.children = [];

    // object this node is representing
    this.obj = obj;

    this.overthrow = function(node, discard=true) {
        //--    Makes the given TreeNode take place of
        //--    this node while this node becomes the
        //--    given node's child. Returns input node.
        //--    Can also specifiy to discard input
        //--    node's children from its previous tree.
        //--

        // delete new node from any its existing tree
        node.delete(discard);

        // if self is not a root node:
        if (this.parent) {

            // set new node as child of my parent
            this.parent.children[this.index] = node;
            node.index = this.index;
            node.parent = this.parent;
        }

        // become a child of new node
        node.in(this);

        // return overthrower (my new parent)
        return node;
    }

    this.replace = function(node) {
        //--    Replaces a node in the tree.
        //--    Resets and returns replaced node.
        //--

        // replace relationship with parent
        if (this.parent) {
            this.parent.children[this.index] = node;
            node.parent = this.parent;
            node.index = this.index;
        ...
        // transfer my children to replacement node children
        for (let child of this.children)
            node.in(child);

        // return resetted replaced node
        this.parent = null;
        this.index = -1;
        this.children = [];
        return this;
    ...
    this.in = function(node) {
        //--    Inserts a new node as this node's child.
    ...
    this.delete = function(discard=true) {
        //--    Removes this node from the tree while
        //--    sending all its children to this node's
        //--    parent. If no parent exists, then
        //--    removes this node from tree completely.
        //--    If discard is true, children will NOT
        //--    be assigned to self's parent node.
        //--    Returns deleted node.
        //--

        // if parent,
        //   set all children's parent to self's parent
        //   add all children to parent's children
        //   also correct the index for each child
        //   remove this node from parent
        //   correct index of parent's children as well
        if (this.parent) {
            ...
            // transfer self's children to parent
            // discard children along with self if discard flag
            if (!discard)
                for (let child of this.children)
                    this.parent.in(child);
        }

        // if no parent,
        //   disconnect children from self
        //   if discard, keep children intact with self
        else {
            if (!discard) {
                for (let child of this.children) {
                    child.parent = null;
        ...
        // - if discarding, keep the children
        // return deleted node
        this.parent = null;
        this.index = -1;
        if (!discard)
...
function Tree(node) {
    ...
    this.levelorder = function(objects_only=false, node=this.root) {
        //--    Returns an array of nodes in level order.
        //--    Can also be specified to return an array of
        //--    objects instead.
        //--
        //--    Time complexity: O(N)
        //--
    ...
    this.preorder = function(objects_only=false, node=this.root, arr=[]) {
        //--    Returns an array of nodes in pre order.
        //--    Can also be specified to return an array of
        //--    objects instead.
        //--
        //--    Time complexity: O(N)
        ...
        for (let n of node.children)
            this.preorder(objects_only, n, arr);

        return arr;
    }

    this.postorder = function(objects_only=false, node=this.root, arr=[]) {
        //--    Returns an array of nodes in post order.
        //--    Can also be specified to return an array of
        //--    objects instead.
        //--
        //--    Time complexity: O(N)
    ...
    this.clear = function() {
        //--    Clears root node from tree.
        //--

        this.root = null;
    }

    this.print = function(node=this.root, level=0) {
        //--    prints out the tree from the given node in
        //--    terms of node level, children, and objs
        //--    (for debugging purposes)
        //--
        ...
        console.log(str);
        for (var i in node.children) {
            this.print(node.children[i], level+1);
        }
    }
}//end Tree
